#!/usr/bin/env python3
"""Plot the Kuavo S53 full-stair phase-two TensorBoard metrics."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--title", default="Kuavo S53 stairs training")
    parser.add_argument("--prefix", default="stairs")
...
def style_axes(axes, milestone: int | None):
    for ax in np.asarray(axes).reshape(-1):
        if milestone is not None:
            ax.axvline(milestone, color="#666666", linestyle="--", linewidth=0.8, alpha=0.55)
        ax.grid(True, alpha=0.22)
        ax.set_xlabel("Training iteration")


def plot_overview(data, output_dir: Path, window: int, title: str, prefix: str, milestone: int | None):
    ...
    style_axes(axes, milestone)
    for ax in axes.reshape(-1):
        ax.legend(loc="best")
    fig.suptitle(f"{title}: training overview", fontsize=16)
    fig.savefig(output_dir / f"{prefix}_training_overview.png", dpi=180)
    plt.close(fig)


def plot_components(data, output_dir: Path, window: int, title: str, prefix: str, milestone: int | None):
    fig, axes = plt.subplots(2, 2, figsize=(14, 9), constrained_layout=True)
    positives = [
    ...
    for tag, label in [
        ("Episode_Reward/motion_anchor_lateral_pos", "Root lateral"),
        ("Episode_Reward/motion_feet_xy", "Foot placement XY"),
        ("Episode_Reward/motion_late_body_pos", "Late body position"),
        ("Episode_Reward/motion_late_feet_pos", "Late foot position"),
    ...
    axes[0, 1].set_title("Full-stair phase-two rewards")
    ...
    for tag, label in [
        ...
        ("Episode_Reward/backward_velocity", "Backward-velocity penalty"),
    ]:
        steps, values = series(data, tag)
    ...
    for ax in axes.reshape(-1):
        ax.legend(loc="best")
    fig.suptitle(f"{title}: reward decomposition", fontsize=16)
    fig.savefig(output_dir / f"{prefix}_reward_components.png", dpi=180)
    plt.close(fig)


def write_csv(data, output_dir: Path, prefix: str):
    ...
    with (output_dir / f"{prefix}_training_metrics.csv").open("w", newline="", encoding="utf-8") as handle:
...
def write_summary(data, run_dirs: list[Path], output_dir: Path, window: int, prefix: str, milestone: int | None):
    tags = [
        "Train/mean_reward",
        "Train/mean_episode_length",
        "Metrics/motion/error_anchor_pos",
    ...
    payload = {
        ...
        "milestone_iteration": milestone,
        "smoothing_window": window,
        "metrics": {tag: metric_summary(data, tag, window) for tag in tags},
    }
    (output_dir / f"{prefix}_training_summary.json").write_text(
...
def main():
    ...
    plot_overview(data, args.output_dir, args.smooth, args.title, args.prefix, args.milestone)
    plot_components(data, args.output_dir, args.smooth, args.title, args.prefix, args.milestone)
    write_csv(data, args.output_dir, args.prefix)
    write_summary(data, args.runs, args.output_dir, args.smooth, args.prefix, args.milestone)
