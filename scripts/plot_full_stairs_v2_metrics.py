...
def style_axes(axes):
    for ax in np.asarray(axes).reshape(-1):
        ax.axvline(29999, color="#666666", linestyle="--", linewidth=0.8, alpha=0.55)
...
def plot_overview(data, output_dir: Path, window: int):
    ...
    for ax in axes.reshape(-1):
        ax.legend(loc="best")
    fig.suptitle("Kuavo S53 full-stair phase-two training overview", fontsize=16)
    fig.savefig(output_dir / "full_stairs_training_overview.png", dpi=180)
    plt.close(fig)
...
def plot_components(data, output_dir: Path, window: int):
    ...
    for ax in axes.reshape(-1):
        ax.legend(loc="best")
    fig.suptitle("Kuavo S53 full-stair reward decomposition", fontsize=16)
    fig.savefig(output_dir / "full_stairs_reward_components.png", dpi=180)
    plt.close(fig)
...
def write_csv(data, output_dir: Path):
    ...
    with (output_dir / "full_stairs_training_metrics.csv").open("w", newline="", encoding="utf-8") as handle:
...
def write_summary(data, run_dirs: list[Path], output_dir: Path, window: int):
    ...
    payload = {
        ...
        "warm_start_iteration": 29999,
        "smoothing_window": window,
        "metrics": {tag: metric_summary(data, tag, window) for tag in tags},
    }
    (output_dir / "full_stairs_training_summary.json").write_text(
