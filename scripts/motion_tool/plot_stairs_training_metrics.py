#!/usr/bin/env python3
"""Merge resumed TensorBoard runs and plot Kuavo S53 stair-training metrics."""
...
def add_resume_markers(axes):
    for ax in np.asarray(axes).reshape(-1):
        for step in (12750, 14750):
            ax.axvline(step, color="#666666", linestyle="--", linewidth=0.8, alpha=0.55)
...
def plot_overview(data, output_dir: Path, window: int):
    ...
    add_resume_markers(axes)
    for ax in axes.reshape(-1):
        ax.legend(loc="best")
    fig.suptitle("Kuavo S53 stair step-1 training overview", fontsize=16)
    fig.savefig(output_dir / "stairs_training_overview.png", dpi=180)
    plt.close(fig)
...
def plot_components(data, output_dir: Path, window: int):
    ...
    axes[0, 1].set_title("Penalty components")
    axes[0, 1].set_ylabel("Episode reward contribution")

    for tag, label in [
        ("Episode_Reward/motion_global_anchor_ori", "Root orientation"),
        ("Episode_Reward/motion_body_ori", "Body orientation"),
    ]:
        steps, values = series(data, tag)
        plot_raw_and_smooth(axes[1, 0], steps, values, label, window)
    axes[1, 0].set_title("Orientation rewards")
    ...
    for ax in axes.reshape(-1):
        ax.legend(loc="best")
    fig.suptitle("Kuavo S53 stair reward decomposition", fontsize=16)
    fig.savefig(output_dir / "stairs_reward_components.png", dpi=180)
    plt.close(fig)
...
def write_csv(data, output_dir: Path):
    ...
    with (output_dir / "stairs_training_metrics.csv").open("w", newline="", encoding="utf-8") as handle:
...
def write_summary(data, run_dirs: list[Path], output_dir: Path, window: int):
    ...
    payload = {
        ...
        "resume_iterations": [12750, 14750],
        "smoothing_window": window,
        "metrics": {tag: metric_summary(data, tag, window) for tag in tags},
    }
    (output_dir / "stairs_training_summary.json").write_text(
