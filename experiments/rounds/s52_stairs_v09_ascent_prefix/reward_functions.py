...
def _reference_foot_phase_masks(
    command: MotionCommand,
    body_indexes: list[int],
    swing_speed_threshold: float,
    relative_speed_margin: float,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Infer swing/stance from the relative speed of a two-foot reference."""
    if len(body_indexes) != 2:
        raise ValueError("reference foot phase rewards require exactly two feet")
    reference_speed = torch.linalg.vector_norm(
        command.body_lin_vel_w[:, body_indexes], dim=-1
    )
    slowest_speed = reference_speed.amin(dim=-1, keepdim=True)
    swing = (reference_speed > float(swing_speed_threshold)) & (
        reference_speed - slowest_speed > float(relative_speed_margin)
    )
    return swing, ~swing


def _scheduled_foot_mask(
    command: MotionCommand,
    foot_frame_ranges: list[list[tuple[int, int]]],
    start_trim: int = 0,
    end_trim: int = 0,
) -> torch.Tensor:
    """Build one explicit phase mask per foot from retargeted motion frames."""
    mask = torch.zeros(
        (command.time_steps.shape[0], len(foot_frame_ranges)),
        dtype=torch.bool,
        device=command.time_steps.device,
    )
    for foot_index, frame_ranges in enumerate(foot_frame_ranges):
        for start, end in frame_ranges:
            first = int(start) + int(start_trim)
            last = int(end) - int(end_trim)
            if first <= last:
                mask[:, foot_index] |= (
                    (command.time_steps >= first) & (command.time_steps <= last)
                )
    return mask


def _scheduled_foothold_targets(
    env: ManagerBasedRLEnv,
    command: MotionCommand,
    body_indexes: list[int],
    foot_frame_ranges: list[list[tuple[int, int]]],
    touchdown_window: int = 0,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Return final footholds, active masks, and normalized swing progress."""
    targets = torch.zeros(
        (command.time_steps.shape[0], len(body_indexes), 3),
        dtype=command.robot_body_pos_w.dtype,
        device=command.robot_body_pos_w.device,
    )
    active = torch.zeros(
        targets.shape[:2], dtype=torch.bool, device=targets.device
    )
    progress = torch.zeros_like(targets[..., 0])
    for foot_index, frame_ranges in enumerate(foot_frame_ranges):
        motion_index = body_indexes[foot_index]
        for start, end in frame_ranges:
            first = int(start)
            last = int(end) + int(touchdown_window)
            in_segment = (
                (command.time_steps >= first) & (command.time_steps <= last)
            )
            target = command.motion.body_pos_w[int(end), motion_index]
            world_target = target.unsqueeze(0) + env.scene.env_origins
            targets[:, foot_index] = torch.where(
                in_segment.unsqueeze(-1), world_target, targets[:, foot_index]
            )
            active[:, foot_index] |= in_segment
            denominator = float(max(int(end) - first, 1))
            segment_progress = (
                (command.time_steps.float() - float(first)) / denominator
            ).clamp(0.0, 1.0)
            progress[:, foot_index] = torch.where(
                in_segment, segment_progress, progress[:, foot_index]
            )
    return targets, active, progress
...
def motion_frame_ranges_anchor_position_l1_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    position_scale: float,
    frame_ranges: list[tuple[int, int]],
) -> torch.Tensor:
    """Penalize global anchor drift without exponential saturation."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    error = torch.abs(command.anchor_pos_w - command.robot_anchor_pos_w)
    normalized = error / float(position_scale)
    return normalized.mean(dim=-1) * _motion_frame_ranges_mask(command, frame_ranges)
...
def motion_frame_ranges_reference_swing_forward_overshoot_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    forward_scale: float,
    ...
    """Keep the active swing foot from crossing a foothold before its reference."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    body_indexes = _get_body_indexes(command, body_names)
    swing, _ = _reference_foot_phase_masks(
        command, body_indexes, swing_speed_threshold, relative_speed_margin
    )
    forward_error = torch.relu(
        command.robot_body_pos_w[:, body_indexes, 0]
        - command.body_pos_w[:, body_indexes, 0]
        - float(allowed_forward)
    )
    normalized = torch.clamp(forward_error / float(forward_scale), max=3.0)
    active = swing.float()
    penalty = (torch.square(normalized) * active).sum(dim=-1) / active.sum(
        dim=-1
    ).clamp_min(1.0)
    return penalty * _motion_frame_ranges_mask(command, frame_ranges)


def motion_frame_ranges_reference_swing_height_shortage_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    height_scale: float,
    allowed_below: float,
    ...
    """Keep the active swing foot above the retargeted clearance corridor."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    body_indexes = _get_body_indexes(command, body_names)
    swing, _ = _reference_foot_phase_masks(
        command, body_indexes, swing_speed_threshold, relative_speed_margin
    )
    height_error = torch.relu(
        command.body_pos_w[:, body_indexes, 2]
        - command.robot_body_pos_w[:, body_indexes, 2]
        - float(allowed_below)
    )
    normalized = torch.clamp(height_error / float(height_scale), max=3.0)
    active = swing.float()
    penalty = (torch.square(normalized) * active).sum(dim=-1) / active.sum(
        dim=-1
    ).clamp_min(1.0)
...
def motion_frame_ranges_reference_swing_contact_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    ...
    """Penalize contact before the active reference swing reaches touchdown."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    body_indexes = _get_body_indexes(command, body_names)
    swing, _ = _reference_foot_phase_masks(
        command, body_indexes, swing_speed_threshold, relative_speed_margin
    ...
    if contact_force.shape[-1] != len(body_indexes):
        raise ValueError("contact sensor body order must match the two reference feet")
    normalized = torch.clamp(
        torch.relu(contact_force - float(contact_force_threshold))
...
def motion_frame_ranges_reference_stance_velocity_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    speed_scale: float,
    swing_speed_threshold: float,
    ...
    """Settle only the reference stance foot, leaving the swing foot unconstrained."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    body_indexes = _get_body_indexes(command, body_names)
    _, stance = _reference_foot_phase_masks(
        command, body_indexes, swing_speed_threshold, relative_speed_margin
    )
    actual_speed = torch.linalg.vector_norm(
        command.robot_body_lin_vel_w[:, body_indexes], dim=-1
    )
    normalized = torch.clamp(actual_speed / float(speed_scale), max=3.0)
    active = stance.float()
    penalty = (torch.square(normalized) * active).sum(dim=-1) / active.sum(
        dim=-1
    ).clamp_min(1.0)
...
def motion_frame_ranges_reference_stance_contact_loss_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    ...
    """Penalize loss of support only on feet marked as stance by the reference."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    body_indexes = _get_body_indexes(command, body_names)
    _, stance = _reference_foot_phase_masks(
        command, body_indexes, swing_speed_threshold, relative_speed_margin
    ...
    if contact_force.shape[-1] != len(body_indexes):
        raise ValueError("contact sensor body order must match the two reference feet")
    normalized = torch.relu(
        float(support_force_threshold) - contact_force
    ) / float(support_force_threshold)
    active = stance.float()
    penalty = (torch.square(normalized) * active).sum(dim=-1) / active.sum(
        dim=-1
    ).clamp_min(1.0)
...
def motion_scheduled_swing_path_tracking_exp(
    env: ManagerBasedRLEnv,
    command_name: str,
    ...
    """Track the collision-free retargeted path throughout each swing segment."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    body_indexes = _get_body_indexes(command, body_names)
    if len(body_indexes) != len(foot_frame_ranges):
        raise ValueError("scheduled swing ranges must match the configured feet")
    active = _scheduled_foot_mask(command, foot_frame_ranges).float()
    error = command.robot_body_pos_w[:, body_indexes] - command.body_pos_w[:, body_indexes]
    scaled_error = (
        torch.sum(torch.square(error[..., :2]), dim=-1) / float(horizontal_std) ** 2
        + torch.square(error[..., 2]) / float(vertical_std) ** 2
    )
    reward = torch.exp(-scaled_error)
    return (reward * active).sum(dim=-1) / active.sum(dim=-1).clamp_min(1.0)


def motion_scheduled_future_foothold_xy_tracking_exp(
    env: ManagerBasedRLEnv,
    command_name: str,
    std: float,
    foot_frame_ranges: list[list[tuple[int, int]]],
    ...
    """Pull the swing foot toward its final foothold as touchdown approaches."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    body_indexes = _get_body_indexes(command, body_names)
    ...
    error = torch.sum(
        torch.square(command.robot_body_pos_w[:, body_indexes, :2] - targets[..., :2]),
        dim=-1,
    )
    weight = active.float() * torch.square(progress)
    reward = torch.exp(-error / float(std) ** 2)
    return (reward * weight).sum(dim=-1) / weight.sum(dim=-1).clamp_min(1.0)


def motion_scheduled_swing_height_shortage_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    height_scale: float,
    allowed_below: float,
    ...
    """Keep each scheduled swing foot above the retargeted vertical corridor."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    body_indexes = _get_body_indexes(command, body_names)
    active = _scheduled_foot_mask(command, foot_frame_ranges).float()
    shortage = torch.relu(
    ...
    normalized = torch.clamp(shortage / float(height_scale), max=3.0)
    return (torch.square(normalized) * active).sum(dim=-1) / active.sum(
        dim=-1
    ).clamp_min(1.0)


def motion_scheduled_swing_early_contact_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    contact_force_threshold: float,
    ...
    """Forbid contact between liftoff and the scheduled touchdown allowance."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    body_indexes = _get_body_indexes(command, body_names)
...
def motion_scheduled_touchdown_position_tracking_exp(
    env: ManagerBasedRLEnv,
    command_name: str,
    horizontal_std: float,
    vertical_std: float,
    touchdown_window: int,
    foot_frame_ranges: list[list[tuple[int, int]]],
    body_names: list[str],
) -> torch.Tensor:
    """Track the fixed final foothold during the post-touchdown settle window."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    body_indexes = _get_body_indexes(command, body_names)
    targets, segment_active, _ = _scheduled_foothold_targets(
        env, command, body_indexes, foot_frame_ranges, touchdown_window
    )
    swing_active = _scheduled_foot_mask(command, foot_frame_ranges)
    touchdown_active = segment_active & ~swing_active
    error = command.robot_body_pos_w[:, body_indexes] - targets
    scaled_error = (
        torch.sum(torch.square(error[..., :2]), dim=-1) / float(horizontal_std) ** 2
        + torch.square(error[..., 2]) / float(vertical_std) ** 2
    )
    active = touchdown_active.float()
    reward = torch.exp(-scaled_error)
    return (reward * active).sum(dim=-1) / active.sum(dim=-1).clamp_min(1.0)


def motion_scheduled_touchdown_contact_loss_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    support_force_threshold: float,
    ...
    """Require load transfer only after the scheduled touchdown frame."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    body_indexes = _get_body_indexes(command, body_names)
    _, segment_active, _ = _scheduled_foothold_targets(
        env, command, body_indexes, foot_frame_ranges, touchdown_window
    )
    active = (segment_active & ~_scheduled_foot_mask(command, foot_frame_ranges)).float()
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    contact_force = (
        contact_sensor.data.net_forces_w_history[:, :, sensor_cfg.body_ids, :]
        .norm(dim=-1)
...
def motion_scheduled_opposite_stance_velocity_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    speed_scale: float,
    foot_frame_ranges: list[list[tuple[int, int]]],
    ...
    """Settle the opposite support foot while one scheduled foot is in swing."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    body_indexes = _get_body_indexes(command, body_names)
    swing = _scheduled_foot_mask(command, foot_frame_ranges)
    active = ((~swing) & swing.any(dim=-1, keepdim=True)).float()
    speed = torch.linalg.vector_norm(
        command.robot_body_lin_vel_w[:, body_indexes], dim=-1
    )
    normalized = torch.clamp(speed / float(speed_scale), max=3.0)
    return (torch.square(normalized) * active).sum(dim=-1) / active.sum(
        dim=-1
    ).clamp_min(1.0)


def motion_scheduled_opposite_stance_contact_loss_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    support_force_threshold: float,
    ...
    """Keep the opposite support foot loaded throughout each scheduled swing."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    body_indexes = _get_body_indexes(command, body_names)
    swing = _scheduled_foot_mask(command, foot_frame_ranges)
    active = ((~swing) & swing.any(dim=-1, keepdim=True)).float()
...
def motion_post_touchdown_sole_support_distance_penalty(
    env: ManagerBasedRLEnv,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    frame_ranges: list[tuple[int, int]],
    ...
    """Penalize uneven sole support only in a short window after touchdown."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    body_indexes = _get_body_indexes(command, body_names)
    sole_points = _body_sample_points_w(command, body_indexes, local_points)
    relative_x = sole_points[..., 0] - env.scene.env_origins[:, None, None, 0]
    ...
    touchdown_window = (
        (contact_force >= float(contact_force_threshold))
        & (contact_time >= float(min_contact_time))
        & (contact_time <= float(max_contact_time))
    )
    normalized = torch.clamp(
        torch.relu(max_gap - float(max_gap_threshold)) / float(gap_scale), max=3.0
    )
    per_foot_penalty = torch.square(normalized) * touchdown_window.float()
    active_feet = touchdown_window.float().sum(dim=-1).clamp(min=1.0)
    penalty = per_foot_penalty.sum(dim=-1) / active_feet
    active = _motion_frame_ranges_mask(command, frame_ranges)

    window_active = touchdown_window.any(dim=-1)
    contacted_gap = torch.where(
        touchdown_window, max_gap, torch.zeros_like(max_gap)
    ).amax(dim=-1)
    command.metrics["post_touchdown_sole_active_fraction"] = (
        touchdown_window.float().mean(dim=-1) * active
    )
    command.metrics["post_touchdown_sole_max_gap"] = contacted_gap * active
    command.metrics["post_touchdown_sole_bad_fraction"] = (
        (
            touchdown_window
    ...
    return penalty * active * window_active.float()
