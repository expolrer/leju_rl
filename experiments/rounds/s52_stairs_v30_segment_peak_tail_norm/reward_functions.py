...
def segment_peak_impact_constraint_probe(
    env,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    ...
    """Keep each descent segment's peak impact active until that segment ends."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    force_norm = contact_sensor.data.net_forces_w_history[
        :, :, sensor_cfg.body_ids, :
    ...
    segment_id = torch.full_like(command.time_steps, -1)
    for index, (frame_start, frame_end) in enumerate(descent_frame_ranges):
        in_segment = (
            (command.time_steps >= int(frame_start))
            & (command.time_steps <= int(frame_end))
        )
        segment_id = torch.where(
            in_segment, torch.full_like(segment_id, index), segment_id
        )
    descent_active = segment_id >= 0

    previous_peak = getattr(
        command, "_s52_segment_running_peak_force", torch.zeros_like(peak_force)
    )
    previous_segment = getattr(
        command, "_s52_segment_running_peak_id", torch.full_like(segment_id, -1)
    )
    previous_frame = getattr(
        command, "_s52_segment_running_peak_frame", torch.full_like(command.time_steps, -1)
    )
    segment_changed = (segment_id != previous_segment) | (
        command.time_steps < previous_frame
    )
    running_peak = torch.where(segment_changed, peak_force, torch.maximum(previous_peak, peak_force))
    running_peak = torch.where(descent_active, running_peak, torch.zeros_like(running_peak))
    command._s52_segment_running_peak_force = running_peak.detach()
    command._s52_segment_running_peak_id = segment_id.detach()
    command._s52_segment_running_peak_frame = command.time_steps.detach().clone()

    hard_impact_active = peak_force >= float(all_course_hard_force_threshold)
    impact_active = descent_active | hard_impact_active
    constrained_force = torch.where(descent_active, running_peak, peak_force)
    impact_excess = torch.relu(
        (constrained_force - float(impact_force_threshold)) / float(impact_force_scale)
    )
    impact_cost = torch.clamp(
        torch.square(impact_excess), max=float(impact_cost_cap)
    ) * impact_active.float()

    command.metrics["s52_segment_peak_constraint_cost"] = impact_cost
    command.metrics["s52_segment_peak_constraint_active"] = impact_active.float()
    ...
    command.metrics["s52_segment_running_peak_contact_force"] = running_peak
    return impact_cost
