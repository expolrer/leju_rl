...
def terminal_route_brake_pseudo_huber_penalty(
    ...
    lateral_excess = torch.relu(torch.abs(route_error[:, 1]) - float(allowed_lateral))
    lateral_loss = _pseudo_huber(lateral_excess / float(lateral_scale))
