...
def terminal_route_tail_pseudo_huber_penalty(
    ...
    return _pseudo_huber(normalized_excess) * tail_mask.float()


def broad_impact_constraint_probe(
    env,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    ...
    """Publish broad descent and emergency all-course impact CMDP costs."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    force_norm = contact_sensor.data.net_forces_w_history[
        :, :, sensor_cfg.body_ids, :
    ...
    command.metrics["s52_impact_constraint_cost"] = impact_cost
    command.metrics["s52_impact_constraint_active"] = impact_active.float()
