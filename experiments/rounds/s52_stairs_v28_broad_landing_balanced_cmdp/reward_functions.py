...
def broad_landing_terminal_constraint_probe(
    env,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    ...
    """Publish scale-balanced descent-impact and terminal-route CMDP costs."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    force_norm = contact_sensor.data.net_forces_w_history[
        :, :, sensor_cfg.body_ids, :
    ...
    active = impact_active | terminal_active
    command.metrics["s52_balanced_constraint_cost"] = combined_cost
    command.metrics["s52_balanced_constraint_active"] = active.float()
