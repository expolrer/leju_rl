from __future__ import annotations

import torch

from isaaclab.managers import SceneEntityCfg
from isaaclab.sensors import ContactSensor
from isaaclab.utils.math import quat_error_magnitude

from leju_robot.tasks.tracking.mdp.commands import MotionCommand


def _body_indexes(command: MotionCommand, body_names: list[str]) -> torch.Tensor:
    indexes = [command.cfg.body_names.index(name) for name in body_names]
    return torch.tensor(indexes, dtype=torch.long, device=command.device)


def _frame_mask(command: MotionCommand, frame_start: int, frame_end: int) -> torch.Tensor:
    return (
        (command.time_steps >= int(frame_start))
        & (command.time_steps <= int(frame_end))
    ).float()


def _pseudo_huber(normalized_error: torch.Tensor) -> torch.Tensor:
    return torch.sqrt(1.0 + torch.square(normalized_error)) - 1.0


def absolute_anchor_route_pseudo_huber_penalty(
    env,
    command_name: str,
    forward_scale: float,
    lateral_scale: float,
    height_scale: float,
) -> torch.Tensor:
    command: MotionCommand = env.command_manager.get_term(command_name)
    scales = torch.tensor(
        [forward_scale, lateral_scale, height_scale],
        dtype=command.robot_anchor_pos_w.dtype,
        device=command.device,
    )
    normalized = (command.robot_anchor_pos_w - command.anchor_pos_w) / scales
    return _pseudo_huber(normalized).mean(dim=-1)


def absolute_feet_route_pseudo_huber_penalty(
    env,
    command_name: str,
    horizontal_scale: float,
    vertical_scale: float,
    body_names: list[str],
    frame_start: int = 0,
    frame_end: int = 10_000,
) -> torch.Tensor:
    command: MotionCommand = env.command_manager.get_term(command_name)
    indexes = _body_indexes(command, body_names)
    error = command.robot_body_pos_w[:, indexes] - command.body_pos_w[:, indexes]
    horizontal = _pseudo_huber(error[..., :2] / float(horizontal_scale)).mean(dim=-1)
    vertical = _pseudo_huber(error[..., 2] / float(vertical_scale))
    return (horizontal + vertical).mean(dim=-1) * _frame_mask(
        command, frame_start, frame_end
    )


def forward_route_overshoot_pseudo_huber_penalty(
    env,
    command_name: str,
    allowed_ahead: float,
    scale: float,
    frame_start: int,
) -> torch.Tensor:
    command: MotionCommand = env.command_manager.get_term(command_name)
    ahead = torch.relu(
        command.robot_anchor_pos_w[:, 0]
        - command.anchor_pos_w[:, 0]
        - float(allowed_ahead)
    )
    return _pseudo_huber(ahead / float(scale)) * _frame_mask(
        command, frame_start, command.motion.time_step_total - 1
    )


def route_velocity_feedback_penalty(
    env,
    command_name: str,
    position_gain: float,
    max_position_correction: float,
    velocity_scale: float,
) -> torch.Tensor:
    """Track reference velocity plus bounded route-error feedback in world XY."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    route_error = command.anchor_pos_w[:, :2] - command.robot_anchor_pos_w[:, :2]
    correction = torch.clamp(
        float(position_gain) * route_error,
        min=-float(max_position_correction),
        max=float(max_position_correction),
    )
    desired_velocity = command.anchor_lin_vel_w[:, :2] + correction
    velocity_error = command.robot_anchor_lin_vel_w[:, :2] - desired_velocity
    return _pseudo_huber(velocity_error / float(velocity_scale)).mean(dim=-1)


def platform_stop_speed_penalty(
    env,
    command_name: str,
    linear_scale: float,
    yaw_scale: float,
    frame_start: int,
    frame_end: int,
) -> torch.Tensor:
    command: MotionCommand = env.command_manager.get_term(command_name)
    linear = _pseudo_huber(command.robot_anchor_lin_vel_w[:, :2] / float(linear_scale)).mean(
        dim=-1
    )
    yaw = _pseudo_huber(command.robot_anchor_ang_vel_w[:, 2] / float(yaw_scale))
    return (linear + 0.25 * yaw) * _frame_mask(command, frame_start, frame_end)


def platform_stationary_feet_penalty(
    env,
    command_name: str,
    body_names: list[str],
    speed_scale: float,
    frame_start: int,
    frame_end: int,
) -> torch.Tensor:
    command: MotionCommand = env.command_manager.get_term(command_name)
    indexes = _body_indexes(command, body_names)
    foot_speed = torch.linalg.vector_norm(
        command.robot_body_lin_vel_w[:, indexes, :], dim=-1
    )
    return _pseudo_huber(foot_speed / float(speed_scale)).mean(dim=-1) * _frame_mask(
        command, frame_start, frame_end
    )


def platform_double_support_hold_reward(
    env,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    support_force_threshold: float,
    position_std: float,
    speed_std: float,
    frame_start: int,
    frame_end: int,
) -> torch.Tensor:
    command: MotionCommand = env.command_manager.get_term(command_name)
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    contact_force = (
        contact_sensor.data.net_forces_w_history[:, :, sensor_cfg.body_ids, :]
        .norm(dim=-1)
        .max(dim=1)[0]
    )
    if contact_force.shape[-1] != 2:
        raise ValueError("platform hold reward requires exactly two foot bodies")
    support = torch.clamp(
        contact_force / float(support_force_threshold), 0.0, 1.0
    ).min(dim=-1)[0]
    route_error = torch.sum(
        torch.square(command.robot_anchor_pos_w - command.anchor_pos_w), dim=-1
    )
    speed_error = torch.sum(torch.square(command.robot_anchor_lin_vel_w), dim=-1)
    return (
        support
        * torch.exp(-route_error / float(position_std) ** 2)
        * torch.exp(-speed_error / float(speed_std) ** 2)
        * _frame_mask(command, frame_start, frame_end)
    )


def platform_anchor_orientation_reward(
    env,
    command_name: str,
    orientation_std: float,
    frame_start: int,
    frame_end: int,
) -> torch.Tensor:
    command: MotionCommand = env.command_manager.get_term(command_name)
    error = quat_error_magnitude(
        command.anchor_quat_w, command.robot_anchor_quat_w
    )
    return torch.exp(-torch.square(error) / float(orientation_std) ** 2) * _frame_mask(
        command, frame_start, frame_end
    )


def terminal_double_support_route_reward(
    env,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    support_force_threshold: float,
    position_std: float,
    speed_std: float,
    frame_start: int,
) -> torch.Tensor:
    command: MotionCommand = env.command_manager.get_term(command_name)
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    contact_force = (
        contact_sensor.data.net_forces_w_history[:, :, sensor_cfg.body_ids, :]
        .norm(dim=-1)
        .max(dim=1)[0]
    )
    support = torch.clamp(
        contact_force / float(support_force_threshold), 0.0, 1.0
    ).min(dim=-1)[0]
    route_error = torch.sum(
        torch.square(command.robot_anchor_pos_w - command.anchor_pos_w), dim=-1
    )
    speed_error = torch.sum(torch.square(command.robot_anchor_lin_vel_w), dim=-1)
    return (
        support
        * torch.exp(-route_error / float(position_std) ** 2)
        * torch.exp(-speed_error / float(speed_std) ** 2)
        * _frame_mask(command, frame_start, command.motion.time_step_total - 1)
    )


def terminal_route_brake_pseudo_huber_penalty(
    env,
    command_name: str,
    allowed_ahead: float,
    position_scale: float,
    position_gain: float,
    max_velocity_correction: float,
    velocity_scale: float,
    yaw_scale: float,
    frame_start: int,
    full_weight_frame: int,
    frame_end: int,
) -> torch.Tensor:
    """Apply dense route recovery and braking only after the final stair descent."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    frame = command.time_steps.float()
    ramp = torch.clamp(
        (frame - float(frame_start))
        / max(float(full_weight_frame - frame_start), 1.0),
        min=0.0,
        max=1.0,
    )
    ramp = ramp * (frame <= float(frame_end)).float()

    route_error = command.robot_anchor_pos_w[:, :2] - command.anchor_pos_w[:, :2]
    forward_ahead = torch.relu(route_error[:, 0] - float(allowed_ahead))
    forward_loss = _pseudo_huber(forward_ahead / float(position_scale))

    velocity_correction = torch.clamp(
        -float(position_gain) * route_error,
        min=-float(max_velocity_correction),
        max=float(max_velocity_correction),
    )
    desired_velocity = command.anchor_lin_vel_w[:, :2] + velocity_correction
    velocity_error = command.robot_anchor_lin_vel_w[:, :2] - desired_velocity
    velocity_loss = _pseudo_huber(velocity_error / float(velocity_scale)).mean(dim=-1)
    yaw_loss = _pseudo_huber(command.robot_anchor_ang_vel_w[:, 2] / float(yaw_scale))
    return (forward_loss + 0.50 * velocity_loss + 0.10 * yaw_loss) * ramp


def terminal_support_floor_route_brake_penalty(
    env,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    support_force_threshold: float,
    allowed_route_error: float,
    position_scale: float,
    outward_velocity_scale: float,
    contact_floor: float,
    frame_start: int,
    full_weight_frame: int,
    frame_end: int,
) -> torch.Tensor:
    """Brake terminal drift with a non-zero gradient in every contact state."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    contact_force = (
        contact_sensor.data.net_forces_w_history[:, :, sensor_cfg.body_ids, :]
        .norm(dim=-1)
        .max(dim=1)[0]
    )
    if contact_force.shape[-1] != 2:
        raise ValueError("terminal support-floor brake requires exactly two foot bodies")
    double_support = torch.clamp(
        contact_force / float(support_force_threshold), 0.0, 1.0
    ).min(dim=-1)[0]

    route_error = command.robot_anchor_pos_w[:, :2] - command.anchor_pos_w[:, :2]
    route_distance = torch.linalg.vector_norm(route_error, dim=-1)
    route_direction = route_error / torch.clamp(route_distance, min=1.0e-6).unsqueeze(-1)
    relative_velocity = (
        command.robot_anchor_lin_vel_w[:, :2] - command.anchor_lin_vel_w[:, :2]
    )
    outward_velocity = torch.relu(torch.sum(relative_velocity * route_direction, dim=-1))

    position_excess = torch.relu(route_distance - float(allowed_route_error))
    position_loss = _pseudo_huber(position_excess / float(position_scale))
    velocity_loss = _pseudo_huber(outward_velocity / float(outward_velocity_scale))

    frame = command.time_steps.float()
    ramp = torch.clamp(
        (frame - float(frame_start))
        / max(float(full_weight_frame - frame_start), 1.0),
        min=0.0,
        max=1.0,
    )
    active = ramp * (frame <= float(frame_end)).float()
    support_blend = float(contact_floor) + (1.0 - float(contact_floor)) * double_support
    command.metrics["s52_terminal_support_floor_route_error"] = route_distance
    command.metrics["s52_terminal_support_floor_outward_velocity"] = outward_velocity
    command.metrics["s52_terminal_double_support"] = double_support
    command.metrics["s52_terminal_support_floor_blend"] = support_blend
    command.metrics["s52_terminal_support_floor_brake_active"] = active
    return (position_loss + 0.50 * velocity_loss) * active * support_blend


def terminal_missing_double_support_penalty(
    env,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    support_force_threshold: float,
    frame_start: int,
    full_weight_frame: int,
    frame_end: int,
) -> torch.Tensor:
    """Keep both feet loaded during terminal hold without touching swing phases."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    contact_force = (
        contact_sensor.data.net_forces_w_history[:, :, sensor_cfg.body_ids, :]
        .norm(dim=-1)
        .max(dim=1)[0]
    )
    if contact_force.shape[-1] != 2:
        raise ValueError("terminal support penalty requires exactly two foot bodies")
    double_support = torch.clamp(
        contact_force / float(support_force_threshold), 0.0, 1.0
    ).min(dim=-1)[0]
    frame = command.time_steps.float()
    active = torch.clamp(
        (frame - float(frame_start))
        / max(float(full_weight_frame - frame_start), 1.0),
        min=0.0,
        max=1.0,
    ) * (frame <= float(frame_end)).float()
    missing_support = (1.0 - double_support) * active
    command.metrics["s52_terminal_missing_double_support"] = missing_support
    command.metrics["s52_terminal_support_penalty_active"] = active
    return missing_support


def windowed_peak_contact_force_barrier(
    env,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    force_threshold: float,
    force_scale: float,
    max_penalty: float,
    frame_start: int,
    frame_end: int,
) -> torch.Tensor:
    """Penalize rare peak foot impacts inside the descent and landing window."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    force_norm = contact_sensor.data.net_forces_w_history[
        :, :, sensor_cfg.body_ids, :
    ].norm(dim=-1)
    peak_force = force_norm.amax(dim=(1, 2))
    normalized_excess = torch.relu(
        (peak_force - float(force_threshold)) / float(force_scale)
    )
    barrier = torch.clamp(torch.square(normalized_excess), max=float(max_penalty))
    return barrier * _frame_mask(command, frame_start, frame_end)


def terminal_route_tail_pseudo_huber_penalty(
    env,
    command_name: str,
    error_threshold: float,
    error_scale: float,
    tail_fraction: float,
    max_normalization_scale: float,
    frame_start: int,
    frame_end: int,
) -> torch.Tensor:
    """Penalize only the worst active terminal-route environments in each batch."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    active = (
        (command.time_steps >= int(frame_start))
        & (command.time_steps <= int(frame_end))
    )
    route_error = torch.linalg.vector_norm(
        command.robot_anchor_pos_w - command.anchor_pos_w, dim=-1
    )
    active_count = int(active.sum().item())
    tail_mask = torch.zeros_like(active)
    cutoff = torch.zeros((), dtype=route_error.dtype, device=route_error.device)
    if active_count > 0:
        tail_count = max(1, int(active_count * float(tail_fraction) + 0.999999))
        masked_error = route_error.masked_fill(~active, float("-inf"))
        tail_values, tail_indexes = torch.topk(masked_error, k=tail_count)
        tail_mask[tail_indexes] = True
        cutoff = tail_values[-1]

    normalized_excess = torch.relu(
        (route_error - float(error_threshold)) / float(error_scale)
    )
    normalization_scale = min(
        float(max_normalization_scale),
        float(route_error.numel()) / float(max(tail_mask.sum().item(), 1)),
    )
    command.metrics["s52_terminal_tail_active"] = tail_mask.float()
    command.metrics["s52_terminal_tail_cutoff"] = cutoff.expand_as(route_error)
    command.metrics["s52_terminal_tail_route_error"] = route_error
    command.metrics["s52_terminal_tail_normalization"] = torch.full_like(
        route_error, normalization_scale
    )
    return (
        _pseudo_huber(normalized_excess)
        * tail_mask.float()
        * normalization_scale
    )


def instant_impact_constraint_probe(
    env,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    descent_frame_ranges: list[tuple[int, int]],
    impact_force_threshold: float,
    impact_force_scale: float,
    impact_cost_cap: float,
    all_course_hard_force_threshold: float,
) -> torch.Tensor:
    """Publish bounded per-step impact cost without replaying old force spikes."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    force_norm = contact_sensor.data.net_forces_w_history[
        :, :, sensor_cfg.body_ids, :
    ].norm(dim=-1)
    peak_force = force_norm.amax(dim=(1, 2))

    descent_active = torch.zeros_like(command.time_steps, dtype=torch.bool)
    for frame_start, frame_end in descent_frame_ranges:
        descent_active |= (
            (command.time_steps >= int(frame_start))
            & (command.time_steps <= int(frame_end))
        )
    hard_impact_active = peak_force >= float(all_course_hard_force_threshold)
    impact_active = descent_active | hard_impact_active
    impact_excess = torch.relu(
        (peak_force - float(impact_force_threshold)) / float(impact_force_scale)
    )
    impact_cost = torch.clamp(
        torch.square(impact_excess), max=float(impact_cost_cap)
    ) * impact_active.float()

    command.metrics["s52_instant_impact_constraint_cost"] = impact_cost
    command.metrics["s52_instant_impact_constraint_active"] = impact_active.float()
    command.metrics["s52_broad_landing_impact_cost"] = impact_cost
    command.metrics["s52_descent_impact_active"] = descent_active.float()
    command.metrics["s52_hard_impact_active"] = hard_impact_active.float()
    command.metrics["s52_instant_peak_contact_force"] = peak_force
    return impact_cost
