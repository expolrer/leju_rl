...
@configclass
class KuavoS52TerminalCurriculumSupportFloorRewardsCfg(
    KuavoS52ScheduledFootholdRewardsCfg
):
    ...
    terminal_support_floor_route_brake = RewTerm(
        func=local_rewards.terminal_support_floor_route_brake_penalty,
        weight=-0.75,
        params={
            "command_name": "motion",
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=FEET),
            "support_force_threshold": 80.0,
            "allowed_route_error": 0.18,
            "position_scale": 0.08,
            "outward_velocity_scale": 0.12,
            "contact_floor": 0.25,
            "frame_start": 1160,
            "full_weight_frame": 1190,
            "frame_end": 1340,
        },
    )
    terminal_missing_double_support = RewTerm(
        func=local_rewards.terminal_missing_double_support_penalty,
        weight=-0.30,
        params={
            "command_name": "motion",
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=FEET),
            "support_force_threshold": 80.0,
            "frame_start": 1260,
            "full_weight_frame": 1280,
            "frame_end": 1340,
        },
    )
    terminal_route_tail = RewTerm(
        func=local_rewards.terminal_route_tail_pseudo_huber_penalty,
        weight=-1.00,
        params={
            "command_name": "motion",
            ...
            "error_scale": 0.12,
            "tail_fraction": 0.20,
            "max_normalization_scale": 4.0,
            "frame_start": 1260,
            "frame_end": 1340,
        },
    )
    instant_constraint_probe = RewTerm(
        func=local_rewards.instant_impact_constraint_probe,
        # RewardManager skips exactly-zero terms. This negligible weight keeps
        # the metrics live while constrained PPO supplies the actor gradient.
        weight=-1.0e-8,
        params={
            "command_name": "motion",
            "sensor_cfg": SceneEntityCfg("contact_forces", body_names=FEET),
            "descent_frame_ranges": [
                (900, 980),
                (981, 1080),
                (1081, 1190),
                (1191, 1258),
            ],
            "impact_force_threshold": 1400.0,
            "impact_force_scale": 800.0,
            "impact_cost_cap": 4.0,
            "all_course_hard_force_threshold": 1800.0,
        },
    )


@configclass
class KuavoS52StairsTerminalCurriculumSupportFloorEnvCfg(
    KuavoS52StairsScheduledFootholdEnvCfg
):
    rewards: KuavoS52TerminalCurriculumSupportFloorRewardsCfg = (
        KuavoS52TerminalCurriculumSupportFloorRewardsCfg()
    )
    ...
    def __post_init__(self):
        ...
        # Keep full-course rollouts while concentrating updates on terminal recovery.
        self.commands.motion.zero_start_fraction = 0.20
        self.commands.motion.terrain_focus_fraction = 0.75
...
@configclass
class KuavoS52StairsTerminalCurriculumSupportFloorEnvCfg_PLAY(
    KuavoS52StairsTerminalCurriculumSupportFloorEnvCfg
):
    def __post_init__(self):
        super().__post_init__()
