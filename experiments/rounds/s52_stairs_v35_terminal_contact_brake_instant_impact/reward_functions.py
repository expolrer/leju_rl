...
def terminal_contact_conditioned_route_brake_penalty(
    env,
    command_name: str,
    sensor_cfg: SceneEntityCfg,
    support_force_threshold: float,
    ...
    """Brake outward terminal drift only while both feet provide support."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    contact_sensor: ContactSensor = env.scene.sensors[sensor_cfg.name]
    contact_force = (
        contact_sensor.data.net_forces_w_history[:, :, sensor_cfg.body_ids, :]
    ...
    if contact_force.shape[-1] != 2:
        raise ValueError("terminal contact brake requires exactly two foot bodies")
    ...
    active = ramp * (frame <= float(frame_end)).float() * double_support
    command.metrics["s52_terminal_contact_route_error"] = route_distance
    command.metrics["s52_terminal_contact_outward_velocity"] = outward_velocity
    command.metrics["s52_terminal_double_support"] = double_support
    command.metrics["s52_terminal_contact_brake_active"] = active
    return (position_loss + 0.50 * velocity_loss) * active
