...
def terminal_predictive_route_tube_penalty(
    env,
    command_name: str,
    ...
    """Anticipate planar terminal drift before the robot leaves the route tube."""
    command: MotionCommand = env.command_manager.get_term(command_name)
    frame = command.time_steps.float()
    ramp = torch.clamp(
        (frame - float(frame_start))
    ...
    predicted_error = route_error + float(prediction_horizon_s) * relative_velocity
    predicted_distance = torch.linalg.vector_norm(predicted_error, dim=-1)
    current_distance = torch.linalg.vector_norm(route_error, dim=-1)
    excess = torch.relu(predicted_distance - float(allowed_route_error))

    command.metrics["s52_terminal_predictive_route_error"] = predicted_distance
    command.metrics["s52_terminal_current_route_error"] = current_distance
    command.metrics["s52_terminal_relative_planar_speed"] = torch.linalg.vector_norm(
        relative_velocity, dim=-1
    )
    command.metrics["s52_terminal_predictive_tube_active"] = active
    return _pseudo_huber(excess / float(error_scale)) * active
