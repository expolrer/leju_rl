...
def _pseudo_huber(normalized_error: torch.Tensor) -> torch.Tensor:
    """Linear-tail robust loss with nonzero gradient at every finite error."""
...
def absolute_route_corridor_reward(
    env,
    command_name: str,
    ...
    command: MotionCommand = env.command_manager.get_term(command_name)
    delta = command.robot_anchor_pos_w - command.anchor_pos_w
    horizontal_error = torch.sum(torch.square(delta[:, :2]), dim=-1)
    vertical_error = torch.square(delta[:, 2])
    return torch.exp(
        -horizontal_error / float(horizontal_std) ** 2
        -vertical_error / float(vertical_std) ** 2
    )
...
def terminal_double_support_route_reward(
    ...
    return (
        support
        * torch.exp(-position_error / float(position_std) ** 2)
